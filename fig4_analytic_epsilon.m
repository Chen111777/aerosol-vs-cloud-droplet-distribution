...
figure('position',[488,49.4,600,800]);
X = record_rbar*10^4;
Y = repmat(record_epss',1,len_t_tick);
eps=sqrt(record_T1+record_T2); % Eq. 14
pic_pcolor_epsilon(X,Y,eps,1)
pic_pcolor_epsilon(X,Y,X.*eps,3)
pic_pcolor_epsilon(X,Y,record_T1,5)
pic_pcolor_epsilon(X,Y,record_T2,7)

simp_T1 = r0_bar^4./X.^4*epsilon0^2; % Eq. 21
simp_T2 = Y.^2./4.*(1-r0_bar^2./X.^2).^2; % Eq. 22
simp_eps=sqrt(simp_T1+simp_T2); % Eq. 23
pic_pcolor_epsilon(X,Y,simp_eps,2)
pic_pcolor_epsilon(X,Y,X.*simp_eps,4)
pic_pcolor_epsilon(X,Y,simp_T1,6)
pic_pcolor_epsilon(X,Y,simp_T2,8)
print('-dpng',gcf,'eps_rbar_epss_4p2','-r450')
...
pst = [0.12,0.775,0.371,0.172;
    0.57,0.775,0.371,0.172;
    0.12,0.543,0.371,0.172;
    0.57,0.543,0.371,0.172;
    0.12,0.311,0.371,0.172;
    0.57,0.311,0.371,0.172;
    0.12,0.079,0.371,0.172;
    0.57,0.079,0.371,0.172];
titlewd = {'(a1) \epsilon';'(a2) Simplified \epsilon';...
    '(b1) \sigma';'(b2) Simplified \sigma';...
    '(c1) T_1';'(c2) Simplified T_1';...
    '(d1) T_2';'(d2) Simplified T_2'} ;
...
elseif i_p==3 || i_p==4 
    contour(X,Y,Z1,0:12,'showtext','on','LineWidth',1.5); 
end
