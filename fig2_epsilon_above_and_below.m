...
load lwcad_exp3_a50_2000_10000_50000
xna=[50,2000,10000,50000]; % [50,100,200,500,1000,2000,5000,10000,20000,50000];
InterFileName = 'dat_fig2_4cases_exp3';
...
ii = 0;
[dat_eps,dat_rm] =deal(nan(nz,5,n_na));
for path1 = paths
    ii = ii+1
...
X=X';
Y=Y';
func_dash_and_solid(repmat(xna',1,10),Y, 1 ,[10,100000],'$N_{a} (cm^{-3})$','log',[0.12,0.46],C2)
...
for im = mnt_range
    ...
    [qc,eps,rm,~,~] = func_get_pro(pathbin,nx,nz);
...
function [qc,eps,rm,sig,N] = func_get_pro(pathbin,nx,nz)
global m r varls
nbins = length(varls);
qc = zeros(nx,nx,nz);
for ibin=1:nbins
